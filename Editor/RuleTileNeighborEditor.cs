using System;
using System.Collections.Generic;
using System.Reflection;
using System.Linq;
using UnityEditorInternal;
using UnityEngine;
using UnityEngine.Tilemaps;
using Object = UnityEngine.Object;

namespace UnityEditor
{
    /// <summary>
    /// The Editor for a RuleTile.
    /// </summary>
    [CustomEditor(typeof(RuleTileNeighbor), true)]
    [CanEditMultipleObjects]
    public class RuleTileNeighborEditor : RuleTileEditor
    {
        private const string s_ArrowNeighbour0 = "";
        private const string s_ArrowNeighbour1 = "";
        private const string s_ArrowNeighbour2 = "";
        private const string s_ArrowNeighbour3 = "";
        private const string s_ArrowNeighbour5 = "";
        private const string s_ArrowNeighbour6 = "";
        private const string s_ArrowNeighbour7 = "";
        private const string s_ArrowNeighbour8 = "";
        
        private static Texture2D[] s_NeighbourArrows;

        /// <summary>
        /// Array of arrow textures used for marking positions for Rule matches
        /// </summary>
        public static Texture2D[] neighbourArrows
        {
            get
            {
                if (s_NeighbourArrows == null)
                {
                    s_NeighbourArrows = new Texture2D[9];
                    s_NeighbourArrows[0] = Base64ToTexture(s_ArrowNeighbour0);
                    s_NeighbourArrows[1] = Base64ToTexture(s_ArrowNeighbour1);
                    s_NeighbourArrows[2] = Base64ToTexture(s_ArrowNeighbour2);
                    s_NeighbourArrows[3] = Base64ToTexture(s_ArrowNeighbour3);
                    s_NeighbourArrows[5] = Base64ToTexture(s_ArrowNeighbour5);
                    s_NeighbourArrows[6] = Base64ToTexture(s_ArrowNeighbour6);
                    s_NeighbourArrows[7] = Base64ToTexture(s_ArrowNeighbour7);
                    s_NeighbourArrows[8] = Base64ToTexture(s_ArrowNeighbour8);
                }
                return s_NeighbourArrows;
            }
        }

        public override void RuleOnGUI(Rect rect, Vector3Int position, int neighbor)
        {
            switch (neighbor)
            {
                case RuleTile.TilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[GetArrowIndex(position)]);
                    break;
                case RuleTileNeighbor.Neighbor.Similar:
                    GUI.DrawTexture(rect, neighbourArrows[GetArrowIndex(position)]);
                    break;
                case RuleTile.TilingRule.Neighbor.NotThis:
                    GUI.DrawTexture(rect, arrows[9]);
                    break;
                default:
                    var style = new GUIStyle();
                    style.alignment = TextAnchor.MiddleCenter;
                    style.fontSize = 10;
                    GUI.Label(rect, neighbor.ToString(), style);
                    break;
            }
        }
    }
}
